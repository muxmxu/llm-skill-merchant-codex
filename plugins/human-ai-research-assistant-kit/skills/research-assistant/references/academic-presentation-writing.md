...
6. **Speaker notes: one line per slide, every slide** (design-time only; the presenter's stage card is Step 8, not these notes). Equation slides narrate the intuition and never read symbols aloud (use the two-part template form). Results slides state the takeaway the audience should leave with, not a repeat of the bullet.
...
## Step 8 — Speaker Deck (downstream stage)

Once the slide page order is frozen, the presenter's stage card (the *speaker deck*: the cue lines the presenter glances at while talking) is produced per `speaker-deck-writing.md`, under the workspace's `DECK_STYLE.md`. The per-slide Speaker note of Rule 6 is a design-time artifact of this mode — it proves a page can be narrated — and is neither the deck nor pasted into it. The deck is indexed by PDF physical page, so it is never written against a draft whose pages may still move. Enter this stage through an explicit request for the speaker card in this mode.
